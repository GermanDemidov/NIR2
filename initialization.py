...
def initialization(filename, fileToMerge):
    synthenyBlocks = {}
    ...
    mainManager = Manager()
    for genom in chrLow:
        for chromosome in chrLow[genom]:
            for tup in chrLow[genom][chromosome]:
                mainManager.add_sb(lowResolutionName, genom, chromosome, tup)
    for genom in chrHigh:
        for chromosome in chrHigh[genom]:
            for tup in chrHigh[genom][chromosome]:
                mainManager.add_sb(highResolutionName, genom, chromosome, tup)
    print mainManager
...
if __name__ == "__main__":
    ...
    initialization(options.filename, options.fileToMerge)
